...
本文档提供 NanoFinance 项目各阶段的复现命令，包括 Tokenizer 训练、预训练、SFT、推理、RAG 部署。**重要声明**：预训练数据相关的部分标记为 `historical artifact unavailable`——原始训练日志不可重新获取，17.68B 总 Token 池等数字为 **[历史自报]**，无法完整复现。本指南提供的命令为项目实际使用的命令模板，具体参数应以仓库内的 manifest/配置文件为准。所有命令均在双 A6000 GPU（48GB each）环境验证。

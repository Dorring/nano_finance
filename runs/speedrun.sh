...
torchrun --standalone --nproc_per_node=1 -m scripts.base_train -- --depth=24 --target-param-data-ratio=9.5 --device-batch-size=8 --run=$WANDB_RUN --window-pattern L
...
curl -L -o $NANOCHAT_BASE_DIR/identity_conversations.jsonl https://karpathy-public.s3.us-west-2.amazonaws.com/identity_conversations.jsonl   #下载一个包含合成身份对话的 JSONL 文件，这些对话用于在 SFT 阶段教授模型一些特殊的对话技能和工具使用能力。这些数据是通过 dev/gen_synthetic_data.py 脚本生成的，文件大小约为 2.3MB。下载完成后，文件将保存在 $NANOCHAT_BASE_DIR/identity_conversations.jsonl 路径下。

"""Generate deterministic predictions for one finance validation checkpoint."""
...
def encode_prompt(tokenizer, messages: list[dict], token_budget: int) -> list[int]:
    if len(messages) != 1 or messages[0].get("role") != "user":
        raise ValueError("finance_eval currently requires exactly one user message")
    bos = tokenizer.get_bos_token_id()
    user_start = tokenizer.encode_special("<|user_start|>")
    user_end = tokenizer.encode_special("<|user_end|>")
    assistant_start = tokenizer.encode_special("<|assistant_start|>")
    content = tokenizer.encode(messages[0]["content"])
    fixed = [bos, user_start, user_end, assistant_start]
    content_budget = token_budget - len(fixed)
    if content_budget < 1:
        raise ValueError("prompt token budget is too small")
    if len(content) > content_budget:
        head = int(content_budget * 0.7)
        content = content[:head] + content[-(content_budget - head):]
    return [bos, user_start, *content, user_end, assistant_start]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--source", choices=("base", "sft"), required=True)
    ...
    if args.temperature != 0:
        parser.error("finance comparison requires deterministic temperature=0")
    return args
...
def main() -> None:
    ...
    if splits != {"val"}:
        raise ValueError(f"refusing non-validation data: splits={splits}")
    ...
    with args.output.open(mode, encoding="utf-8") as handle:
        for index, example in enumerate(examples, start=1):
            if example["id"] in completed:
                continue
            prompt = encode_prompt(tokenizer, example["messages"], prompt_budget)
